...
export function DayView({
  ...
  return (
    <div className="contents" data-slot="day-view">
      ...
      <div
        className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-muted scrollbar-track-transparent border-t border-border">
        <div className="grid grid-cols-[3rem_1fr] sm:grid-cols-[4rem_1fr] min-h-max bg-background relative z-0">
          <div className="bg-background sticky left-0 z-30 border-r border-border">
            {hours.map((hour, index) => (
              <div
                className="relative h-[var(--week-cells-height)] border-border border-b last:border-b-0"
                key={hour.toString()}>
                {index > 0 && (
                  <span
                    className="-top-3 absolute left-0 flex h-6 w-16 max-w-full items-center justify-end bg-background pe-2 text-[10px] text-muted-foreground sm:pe-4 sm:text-xs">
                    {format(hour, "h a")}
                  </span>
                )}
              </div>
